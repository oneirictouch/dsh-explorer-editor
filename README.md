...
安装后**完全退出并重启桌面端**（退出应用，不是关窗口），`dsh-explorer-editor` 才会出现在侧边栏底部"文件"按钮和设置 → 插件列表中。
...
- **编码自动识别**：文本文件优先按 UTF-8 读取，UTF-8 非法时自动回退 **GBK/GB2312**（Windows 生成的日志/导出常见），中文不再乱码；编辑保存后文件统一转为 UTF-8
